...
class DTDDataProvider:
    def __init__(self,image_list_file,save_path=None, train_batch_size=32, test_batch_size=200, valid_size=None, n_worker=32,
                 resize_scale=0.08, distort_color=None, image_size=224,
                 num_replicas=None, rank=None):
        """
        Args:
        ...
        self.labels = labels
        test_dataset = DTDDataProvider(data_dir=data_dir,
        ...
        self.test = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=test_batch_size,
    ...
    def __getitem__(self, index):
        ...
        torch.nn.functional.one_hot(label, num_classes=24)
        if self.transform is not None:
            image = self.transform(image)
        return image, torch.FloatTensor(label),image_name[:-25]
